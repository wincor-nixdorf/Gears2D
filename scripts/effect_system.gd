# effect_system.gd - полная версия с новым методом
...
func get_resistance_bonus(target: Object) -> int:
	if not _modifiers.has(target):
		return 0
	var total = 0
	for entry in _modifiers[target]:
		if entry.tag == "resistance_bonus":
			total += entry.data.get("amount", 0)
	return total

# Новый метод: проверяет наличие prevent_trigger и потребляет его
func has_prevent_trigger(target: Object) -> bool:
	if not _modifiers.has(target):
		return false
	var entries = _modifiers[target]
	for i in range(entries.size() - 1, -1, -1):
		if entries[i].tag == "prevent_trigger":
			entries.remove_at(i)
			if entries.is_empty():
				_modifiers.erase(target)
			GameLogger.debug("EffectSystem: consumed prevent_trigger for %s" % target)
			return true
	return false
