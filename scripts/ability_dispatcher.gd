...
func _connect_signals():
	...
	EventBus.gear_rotated.connect(_on_gear_rotated)
...
func _on_gear_placed(gear: Gear, cell: Cell):
	...
	for enemy_gear in game_manager.board_manager.get_all_gears():
		if enemy_gear.owner_id == enemy_id and GameState.effect_system.has_modifier(enemy_gear, "time_swarm_source"):
			GameState.effect_system.add_modifier(gear, enemy_gear, "no_auto_tick")
...
func _handle_ability(ability: Ability, base_context: Dictionary):
	if ability.target_type != GameEnums.TargetType.NO_TARGET:
		...
		if possible_targets.size() == 1 and ability.target_type != GameEnums.TargetType.ANY:
			...
			# Принудительно сбрасываем режим выбора цели, если он был активен
			if target_selector.is_waiting:
				target_selector.cancel_selection()
			else:
